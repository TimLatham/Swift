...
if yesCount > noCount {
    ...
    print("There were \(yesCount) yes votes, versus \(noCount) no votes")
} else {
    print("The mascot stays!")
    print("There were \(yesCount) yes votes, versus \(noCount) no votes")
}
...
let issueList = [shouldMascotChangeVotes, shouldInstallCoffeeVendingMachineVotes,shouldHaveMorePollOptionsVotes]
let issueNames = ["Mascot Change", "Install Coffee Vending", "More Poll Options"]
...
for issue in issueList {
    yesCount = 0
    noCount = 0
    var voteItem = issueNames[listCounter]
    for vote in issue {
        if vote == true {
            yesCount += 1
        } else {
    ...
    if yesCount > noCount {
        print("The issue \(voteItem) passed")
        print("There were \(yesCount) yes votes, versus \(noCount) no votes")
    } else {
        print("The issue \(voteItem) failed to achieve a majority")
        print("There were \(yesCount) yes votes, versus \(noCount) no votes")
    }
    listCounter += 1
}
