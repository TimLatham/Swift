 ...
 */
let nameInCaps = "Fritz Alfredo"
let name = "fritz alfredo"
print(nameInCaps == name ? "The two strings are equal":"The two strings are not equal")
/*:
 ...
 */
print(nameInCaps.lowercased() == name.lowercased() ? "\(nameInCaps.lowercased()) and \(name.lowercased()) are the same":"\(nameInCaps.lowercased()) and \(name.lowercased()) are not the same")
...
print(junior.hasSuffix("Jr.") ? "We found a second generation name!":"")
...
print(textToSearchFor.lowercased().contains(textToSearchFor.lowercased()) ? "I found it!":"Nope")
/*:
